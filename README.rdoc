...
5a) Register the user
  registration_manager = YodleeApi::UserRegistration.new(client.cobrand_context, user_creds)
  registration_manager.register

5b) If you already have a user, log in instead
  login_manager = YodleeApi::UserLogin.new(client.cobrand_context, user_creds) # you don't have to pass in email to user credentials for login
  login_manager.login
...
7) Access the sites list. The sites will be stored an array of hashes with keys :content_service_id, :site_name, :organization_name and :login_form
